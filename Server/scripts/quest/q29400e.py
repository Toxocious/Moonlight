...
endTime = data[2].split("=")[1]
state = int(data[3].split("=")[1])
...
def run():
    if state == 1:
        ...
        sm.setQRValue(QUEST, "mobCount=" + str(mobCount) + ";startTime=" + startTime + ";endTime=" + date + ";state=1")
        sm.completeQuest(QUEST)
    else:
