...
public class Server extends Properties {
  ...
  private Map<String, Tuple<Integer, FileTime>> authTokens =
      new ConcurrentHashMap<>();
  private Set<Integer> users =
      new HashSet<>(); // just save the ids, no need to save the references
  ...
  public List<World> getWorlds() { return worldList; }
  ...
  private void init(String[] args) {
    ...
    log.info("Loaded Hibernate in " + (System.currentTimeMillis() - startNow) +
             "ms");
    ...
    worldList.add(new World(ServerConfig.WORLD_ID, ServerConfig.SERVER_NAME,
                            GameConstants.CHANNELS_PER_WORLD,
                            ServerConfig.EVENT_MSG));
    ...
    initCashShop();
    log.info("Loaded CashShop in " +
             (System.currentTimeMillis() - startCashShop) + "ms");
    ...
    log.info(String.format("Finished loading server in %dms",
                           System.currentTimeMillis() - startNow));
    new Thread(() -> {
      // inits the script engine
      log.info(String.format("Starting %s script engine.",
                             ScriptManagerImpl.SCRIPT_ENGINE_NAME));
    }).start();

    this.online = true;
  }

  public ScheduledFuture getShutdownTimer() { return shutdownTimer; }

  public void setShutdownTimer(ScheduledFuture shutdownTimer) {
    this.shutdownTimer = shutdownTimer;
  }

  public boolean isShutdownFromCommand() { return shutdownFromCommand; }
  ...
  public void sendShutdownMessage(int time) {
    ...
    String timeMsg =
        time + (!isShutdownFromCommand() ? " seconds." : " minutes. ");
    String end = "Please log off safely before the server shuts down.";
    getWorldById(1).broadcastPacket(
        UserLocal.chatMsg(ChatType.Notice2, "[Notice] " + msg + timeMsg + end));
    getWorldById(1).broadcastPacket(UserLocal.addPopupSay(
        9010063, 10000,
        "#e#b[Notice]#k#n " + msg + "#e#r" + timeMsg + "#k#n" + end,
        "FarmSE.img/boxResult"));
    ServerConfig.SERVER_MSG = msg + timeMsg + end;
    getWorldById(1).broadcastPacket(WvsContext.broadcastMsg(
        BroadcastMsg.slideNotice(ServerConfig.SERVER_MSG, true)));
  }
  ...
  private void checkAndCreateDat() {
    ...
    if (!exists) {
      log.info("Dat files cannot be found (at least not the equip dats). All "
               + "dats will now be generated. This may take a long while.");
    ...
    for (Class c : DataClasses.dataClasses) {
      for (Method method : c.getMethods()) {
        ...
        if (annotation != null) {
          ...
          if (exists) {
            log.info(String.format("Took %dms to load from %s", total,
                                   file.getName()));
          } else {
            log.info(String.format("Took %dms to load using %s", total,
                                   method.getName()));
          }
        }
      }
  ...
  public CashShop getCashShop() { return this.cashShop; }

  public void addUser(User user) { users.add(user.getId()); }
  ...
  public boolean isOpcodeEnc() { return opcodeEnc; }

  public void setOpcodeEnc(boolean opcodeEnc) { this.opcodeEnc = opcodeEnc; }
  ...
  public void addAuthToken(byte[] token, int userID) {
    ...
    FileTime expiryDate = FileTime.fromDate(
        LocalDateTime.now().plusMinutes(ServerConstants.TOKEN_EXPIRY_TIME));
  ...
  public void cancelShutdown() {
    if (shutdownFutures.size() > 0) {
      ...
      for (World world : getWorlds()) {
        world.broadcastPacket(UserLocal.chatMsg(
            ChatType.Notice2, "[Notice] Cancelled shutdown."));
      }
    }
  }

  public static void main(String[] args) { getInstance().init(args); }
}
